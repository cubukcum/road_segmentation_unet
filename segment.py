...
def run(model: str, display_mode: str, num_threads: int, enable_edgetpu: bool,
  ...
  while cap.isOpened():
    ...
    counter += 1
    image = cv2.flip(image, 1) # MAY NOT NEED IF USING AN USB CAMERA
...
# Defining class color mappings
  ...
  colored_labels = [
        processor.ColoredLabel(category_name='Road', color=Road / 255.0),
        processor.ColoredLabel(category_name='Lane', color=Lane / 255.0),
        processor.ColoredLabel(category_name='Undrivable', color=Undrivable / 255.0),
        processor.ColoredLabel(category_name='Movable', color=Movable / 255.0),
        processor.ColoredLabel(category_name='Car', color=Car / 255.0),
    ]

    # Convert the colored labels to a colormap for visualization
  colors = [label.color for label in colored_labels]
  cmap = ListedColormap(colors)

    # Convert segmentation_map_image to RGB for compatibility with the colormap
  segmentation_map_rgb = cmap(segmentation_map_image)
